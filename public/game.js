...
function connect(){
  ...
  ws.onmessage=e=>{const m=JSON.parse(e.data);
    if(m.type==="joined"){id=m.id;roomCode=m.room;round=m.round;$("roomLabel").textContent="ROOM "+roomCode;$("round").textContent="ROUND "+round;$("menu").classList.add("hidden");$("game").classList.remove("hidden");}
  ...
  ws.onclose=()=>{if(!id)alert("Connection failed. Start the server and try again.");};
}
function start(type){
  const name=$("name").value.trim()||"Player";
  connect(); setTimeout(()=>ws.send(JSON.stringify(type==="create"?{type,name}:{type,name,room:$("room").value})),300);
}
$("create").onclick=()=>start("create");$("join").onclick=()=>start("join");
...
setInterval(()=>{if(!ws||ws.readyState!==1)return;let dx=0,dy=0;if(keys.w||keys.arrowup)dy--;if(keys.s||keys.arrowdown)dy++;if(keys.a||keys.arrowleft)dx--;if(keys.d||keys.arrowright)dx++;if(dx||dy)ws.send(JSON.stringify({type:"move",dx,dy}))},50);

function draw(){
 ctx.clearRect(0,0,800,600);ctx.fillStyle="#0b1020";ctx.fillRect(0,0,800,600);
 ctx.strokeStyle="#17213a";for(let x=0;x<800;x+=40){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,600);ctx.stroke()}for(let y=0;y<600;y+=40){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(800,y);ctx.stroke()}
 ...
 for(const p of players){
   ...
   ctx.beginPath();for(let i=0;i<6;i++){let a=i*Math.PI/3;let x=Math.cos(a)*19,y=Math.sin(a)*19;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath();
